...
class dMap:
    ...
    def __init__(self):
       self.roomList=[]
       self.cList=[]
       self.nstairsup = randrange(3)+1
       self.nstairsdown = randrange(3)+1
    ...
    def placestairs(self, xm, ym):
        ...
        while w < 3:
            ...
            print x
            print y
            if self.mapArr[x][y] == 0:
                self.mapArr[x][y] = 6
                self.start_x = x
                self.start_y = y
                self.logging.log('Up Staircase Created at: ' + str(x) + ' ' + str(y))
                w += 1
        while z < 3:
            x=randrange(xm)
            y=randrange(ym)
            if self.mapArr[x][y] == 0:
                self.mapArr[x][y] = 7
                self.logging.log('Down Staircase Created at: ' + str(x) + ' ' + str(y))
                z += 1
